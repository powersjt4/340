...
app.use('/addpi', require('./addpi.js'))
...
app.get('/getMenuDB', function (req, res) {
  var context = {}
...
app.post('/insertmenu', function (req, res) {
  ...
  mysql.pool.query('INSERT INTO menu(`restaurant_name`,`menu_meal`) VALUES (?,?)', [postData.restaurant_name, postData.menu_meal], function (err, insertResult) {
    if (err) {
...
app.post('/insertitem', function (req, res) {
  ...
  mysql.pool.query(sql, inserts, function (err, insertResult) {
    ...
    var sql2 = 'SELECT i.id, i.name, i.price, i.description, m.name AS item_meal, pi.name AS primary_ingredient FROM item i INNER JOIN meal m ON i.item_meal = m.id INNER JOIN primary_ingredient pi ON i.primary_ingredient = pi.id WHERE i.name = ?;'
...
app.get('/getMenuItems/:id', function (req, res) {
  ...
  mysql.pool.query(sql1, inserts, function (error, results, fields) {
    ...
    var sql2 = 'SELECT i.id, i.name, i.price, i.description, m.name AS item_meal, pi.name AS primary_ingredient FROM item i INNER JOIN meal m ON i.item_meal = m.id INNER JOIN primary_ingredient pi ON i.primary_ingredient = pi.id;'
...
app.listen(app.get('port'), function () {
  console.log('Express started on port number ' + app.get('port') + '; press Ctrl-C to terminate.')
})
