...
document.getElementById('submitWorkout').addEventListener('click', function (event) {
  ...
  if (document.getElementById('weightType1').checked) { data.lbs = document.getElementById('weightType1').value } else { data.lbs = document.getElementById('weightType2').value }
  ...
  req.addEventListener('load', function () {
    if (req.status >= 200 && req.status < 400) {
      var response = JSON.parse(req.responseText)
      if (response) { addToList(response) }
    } else {
...
function addToList (newWorkout) {
  if (newWorkout.lbs == 1) { newWorkout.lbs = 'lbs' } else { newWorkout.lbs = 'kgs' }
...
function editItem () {
  ...
  req.addEventListener('load', function () {
    if (req.status >= 200 && req.status < 400) {
      ...
      if (response[0].lbs == 1) { document.getElementById('weightType1').checked = true } else { document.getElementById('weightType2').checked = true }
      ...
      document.getElementById('editWorkout').addEventListener('click', function (event) {
        ...
        if (document.getElementById('weightType1').checked) { data.lbs = document.getElementById('weightType1').value } else { data.lbs = document.getElementById('weightType2').value }
        data.id = id
        if (data != '') { req.open('POST', '/update', true) }
   						req.setRequestHeader('Content-Type', 'application/json')
        req.addEventListener('load', function () {
          if (req.status >= 200 && req.status < 400) {
            location.reload()
